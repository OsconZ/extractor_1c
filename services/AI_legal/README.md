...
## Эндпойнты
- `POST /api/sections/full` — принимает файл (`file`) c JSON вида `{\"part_0\": \"...\", ..., \"part_16\": \"...\"}` и возвращает агрегированный HTML‑отчёт вместе с оценками (`overall_score`, `inaccuracy`, `red_flags`).
- `POST /api/sections/full-prepared` — возвращает расширенный ответ с секциями, html‑отчётом, полным текстом договора и спецификацией.
